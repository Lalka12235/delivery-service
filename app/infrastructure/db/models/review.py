...
class ReviewModel(Base):
    ...
    user_id: Mapped[uuid.UUID] = mapped_column(ForeignKey('users.id'),UUID(as_uuid=True),nullable=True)
    order_id: Mapped[uuid.UUID] = mapped_column(ForeignKey('orders.id'),UUID(as_uuid=True),nullable=True)
    courier_id: Mapped[uuid.UUID] = mapped_column(ForeignKey('couriers.id'),UUID(as_uuid=True),nullable=True)
